...
def load_json(file_path: str, max_samples: Optional[int] = None, skip_samples: int = 0) -> List[Dict[str, Any]]:
    """
    ...
    if skip_samples < 0:
        raise ValueError("skip_samples must be a non-negative integer")
        
    data = []
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            if file_path.endswith('.jsonl'):
                # Skip the first skip_samples lines
                for _ in range(skip_samples):
                    next(f, None)
                
                # Now read max_samples lines
                samples_loaded = 0
                for line in f:
                ...
                if file_content:
                    ...
                    if isinstance(loaded_data, list):
                        end_idx = None if max_samples is None else skip_samples + max_samples
                        data = loaded_data[skip_samples:end_idx]
                    else:
                        data = [loaded_data] if skip_samples == 0 else []
    ...
    logger.info(f"Loaded {len(data)} samples from {file_path} (skipped {skip_samples}).")
    return data

def load_json_old(file_path: str, max_samples: Optional[int] = None) -> List[Dict[str, Any]]:
    """
    Load data from a JSON or JSONL file.
